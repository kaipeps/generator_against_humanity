...
def get_saved_cards(user_id):
    saved_cards = sql(f'SELECT color, text, author_name FROM cards INNER JOIN saves ON saved_card_id = card_id WHERE saved_user_id = {user_id}')
    return saved_cards

def create_new_card(color, text, author_id, author_name, pick, public_card):
    sql('INSERT INTO cards (color, text, author_id, author_name, pick, public_card) VALUES (%s, %s, %s, %s, %s, %s) RETURNING *', [color, text, author_id, author_name, pick, public_card])

def save_card(card_id, user_id):
...
def delete_card(card_id):
    sql(f'DELETE FROM cards WHERE id = {card_id} RETURNING *')
